...
public class SupplierDao implements SupplierDaoInt{
    ...
    public void update(Connection con, Supplier supplier) throws SQLException{
        PreparedStatement ps = null;
        try{
            ps = con.prepareStatement("UPDATE suppliers SET SupplierName=?, ContactPerson=?, Address=?, "
    ...
    public void create(Connection con, Supplier supplier) throws SQLException{
        ...
        ResultSet rs = null;
        try{
            ps = con.prepareStatement("INSERT INTO suppliers(SupplierName, ContactPerson, Address, City,"
                        + "PostCode, Country, Phone) VALUES(?,?,?,?,?,?,?)");
            ps.setString(1, supplier.getSupplierName());
            ps.setString(2, supplier.getContactPerson());
            ps.setString(3, supplier.getAddress());
            ps.setString(4, supplier.getCity());
